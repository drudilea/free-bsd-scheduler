...
% https://github.com/drudilea/freebsd-src/compare/release/11.0.0...drudilea:freebsd-src:DrudiGoldmanPI/update_petriNetScheduler-11.0.0

Como se explico previamente, planificar es decidir sobre el orden y la duracion de la ejecucion de los distintos hilos del sistema. Desde siempre, los sistemas operativos han basado sus decisiones a estadisticas sobre el funcionamiento previo, es decir, tomando tales decisiones, me fue de tal manera.

Los puntos claves donde hay que tomar decisiones son los momentos en donde se decide que un hilo tiene que frenar su uso del CPU para dejarselo a otro y asi lograr la ilusion de que todos ellos se ejecutan de forma concurrente. Basado en lo explicado sobre el funcionamiento del planificador 4BSD, esto se reduce a la des-encolar un hilo de una queue de hilos RUNNABLE y pasarlo a ejecucion, dejando el hilo que se encontraba haciendo uso de dicha cpu en una de estas colas de ejecucion.

Es aqui donde se encontró una oportunidad de mejora. Modelar este sistema de encolado y desencolado de los hilos mediante una Red de Petri.

% El planificador tradicional 4BSD cuenta con una cola por procesador + una cola global. Ver en el codigo viejo (link de arriba), cuando es que querias que un hilo se vaya a la cola global y porque; y cuando una cola especifica de procesador.

% - Donde  fue el cambio PRINCIPAL (decision del encolado)
% - Funciones principales modificadas para adaptarse a la red de petri (en terminos de planificacion)
% - Funciones principales modificadas para adaptarse a la red de petri (en terminos de codigo)


% \subsubsection{Puntos de interes para nuestra tesis}
...
% LO DE ABAJO NO LO BORRAMOS PARA VER SI VA ALGO O NO (DEPRECATED)
