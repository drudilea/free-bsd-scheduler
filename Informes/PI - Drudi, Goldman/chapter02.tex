...
Los procesos en FreeBSD pueden adoptar varios estados, cada uno indicando su actividad y situación actual:

\begin{itemize}
    \item \uppercase{New}: Cuando se crea un proceso mediante la llamada al sistema fork(), se inicia en este estado. En este estado, el proceso aún no ha sido completamente configurado y está a la espera de los recursos necesarios para comenzar su ejecución.
    \item \uppercase{Normal}: Una vez que se han asignado los recursos necesarios, el proceso pasa a este estado, lo que indica que está listo para ejecutarse. Los procesos en este estado pueden cambiar entre los siguientes subestados:
        \begin{itemize}
            \item \uppercase{Runnable}: El proceso está listo para ser ejecutado o actualmente se está ejecutando en el procesador.
            \item \uppercase{Sleeping}: El proceso está temporalmente suspendido, esperando la ocurrencia de un evento, como la llegada de datos o una señal, antes de poder continuar su ejecución.
            \item \uppercase{Stopped}: El proceso está temporalmente detenido, ya sea por una senal enviada desde otro proceso o por el proceso padre.
        \end{itemize}
    \item \uppercase{Zombie}: Después de que un proceso ha completado su ejecución, entra en este estado. En este estado, el proceso aún tiene una entrada en la tabla de procesos del sistema, pero todos sus recursos han sido liberados y está a la espera de que su proceso padre recoja su estado de salida.
\end{itemize}
