...
La planificación es un componente esencial de cualquier sistema operativo, encargado de asignar tiempo de procesamiento a los diferentes hilos y procesos en ejecución. En este capítulo se presentan los conceptos necesarios para comprender la planificación a corto plazo en FreeBSD y, en particular, el funcionamiento del planificador 4BSD. Como ya se indicó en el capítulo 1, este trabajo continúa una investigación previa\cite{bib1}, donde se propuso modelar parte del planificador mediante Redes de Petri.\par

Primero se describen los conceptos fundamentales de procesos e hilos, incluyendo su estructura, sus estados y su relación con las colas de ejecución. Luego se repasan las operaciones principales del planificador, como el cambio de contexto, el encolado de hilos, la selección de procesador y la remoción de elementos de la cola. Estos puntos permiten entender sobre qué comportamiento concreto se apoya el modelo utilizado en el proyecto.\par

Finalmente se introducen las nociones de Redes de Petri que se usan a lo largo del informe. No se busca desarrollar una teoría completa sobre el tema, sino dejar explícitos los elementos que aparecen luego en el diseño y en la implementación: plazas, transiciones, tokens, marcado, sensibilización, disparo de transiciones, matrices y arcos inhibidores.\par
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|p{0.8\textwidth}|}
        ...
        \hline
        NORMAL          & Ejecución activa, sus hilos alternarán entre \textit{RUNNABLE}, \textit{SLEEPING} o \textit{STOPPED} \\
        \hline
...
Estas reglas no representan solamente preferencias de balanceo de carga. En particular, los hilos \textit{bound} y \textit{pinned}, y ciertos casos de afinidad, limitan el conjunto de procesadores válidos para ejecutar un hilo. Por este motivo, una decisión de planificación que envíe estos hilos a la cola global o a una cola per-CPU no permitida puede violar supuestos internos del \textit{kernel}, ya que la cola global puede ser consumida por cualquier CPU disponible.\par

\section{Redes de Petri}

Las Redes de Petri son una herramienta formal de modelado para sistemas de eventos discretos, especialmente útil cuando el comportamiento que se quiere representar depende de estados, eventos y concurrencia\cite{bib8,bib9}. En el contexto de este trabajo, esa característica resulta relevante porque el planificador no ejecuta una única secuencia lineal de pasos, sino que debe administrar hilos, colas y procesadores que cambian de estado en distintos momentos.\par

Una Red de Petri se compone principalmente de plazas, transiciones, arcos y tokens. Las plazas representan condiciones o estados del sistema; las transiciones representan eventos que pueden modificar esos estados; los arcos indican qué plazas participan como entrada o salida de cada transición; y los tokens representan la presencia de una condición en un momento determinado. El conjunto de tokens distribuidos sobre las plazas recibe el nombre de marcado, y describe el estado actual de la red\cite{bib7}.\par

Una transición está sensibilizada cuando las plazas de entrada tienen los tokens necesarios para permitir su ejecución. Al dispararse, la transición consume tokens de sus plazas de entrada y produce tokens en sus plazas de salida. De esta forma, el cambio de estado queda representado por una operación sobre el marcado. Esta idea es la que permite relacionar el comportamiento del planificador con operaciones concretas dentro de la red: por ejemplo, encolar un hilo, retirarlo de una cola, asignarlo a una CPU o liberar el procesador.\par

En una implementación por software, este comportamiento puede representarse mediante matrices. La matriz de incidencia describe cómo cambia el marcado cuando se dispara una transición: qué plazas pierden tokens y qué plazas los reciben. En este trabajo también se utiliza una matriz de inhibición, necesaria para representar arcos inhibidores. Un arco inhibidor permite bloquear una transición cuando una plaza contiene tokens, lo cual resulta útil para modelar condiciones de exclusión o de control\cite{bib7}. Por ejemplo, si una CPU se encuentra inhibida para recibir trabajo, una transición de encolado asociada a esa CPU no debería quedar disponible.\par

Además del comportamiento dinámico, las Redes de Petri permiten analizar ciertas propiedades del modelo. Una red acotada impide que una plaza acumule una cantidad ilimitada de tokens; una red segura es aquella donde cada plaza puede contener, como máximo, un token; y una red libre de interbloqueos evita estados en los que el sistema queda sin posibilidad de avanzar. Estas propiedades no sustituyen las pruebas sobre el \textit{kernel}, pero sirven como una primera forma de razonamiento sobre el diseño del modelo antes de llevarlo al código\cite{bib7}.\par

Para conectar el modelo con el planificador se utilizan dos niveles de representación. Por un lado, existe una red asociada a cada hilo, donde se representan estados como listo para ejecutar, encolado, ejecutando o esperando un evento. Por otro lado, existe una red de recursos, que representa el estado de las CPU y sus colas. La relación entre ambas se expresa mediante transiciones jerárquicas: cuando se dispara una transición en la red de recursos, también debe dispararse la transición correspondiente en la red del hilo. Esta separación permite modelar el estado local de cada hilo sin perder la visión global de los recursos del sistema.\par

Sobre esta base teórica se apoya el proyecto integrador previo y también las extensiones desarrolladas en este trabajo. En particular, la incorporación de nuevas funcionalidades al planificador se analiza observando si pueden representarse como nuevas plazas, transiciones y relaciones dentro de la red, o si requieren información adicional que no está contenida directamente en el marcado.\par

\section{Conceptos del proyecto integrador previo}

Con estos conceptos básicos de Redes de Petri ya presentados, se puede entender con mayor claridad el trabajo integrador previo sobre el que se apoya esta tesis. En esta sección se retoman sus decisiones principales y el modelo final que luego fue tomado como punto de partida para desarrollar las nuevas extensiones.\par
